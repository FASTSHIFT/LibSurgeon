...
def generate_master_header_for_archive(archive_name: str, include_dir: str):
    """
    Generate a master header file for the archive.
    Header files are already generated directly in include_dir by Ghidra script.
    """
    # Find all .h files in include_dir
    header_files = sorted(glob.glob(os.path.join(include_dir, "*.h")))

    if not header_files:
        log_warn("No header files found in include directory")
        return

    # Collect header basenames
    headers = [os.path.basename(h) for h in header_files]

    # Generate master header
    safe_name = archive_name.replace("-", "_").replace(".", "_")
    master_header = os.path.join(include_dir, f"{safe_name}_all.h")
    guard_name = f"_{safe_name.upper()}_ALL_H_"

    with open(master_header, "w") as f:
        f.write("/**\n")
        f.write(f" * Master Header for {archive_name}\n")
        f.write(" *\n")
        f.write(" * Includes all decompiled function declarations.\n")
        ...
        # Include _types.h if it exists
        if "_types.h" in headers:
            f.write('#include "_types.h"\n\n')

        f.write("/* Individual module headers */\n")
        for header in sorted(headers):
            if header != "_types.h" and not header.endswith("_all.h"):
                f.write(f'#include "{header}"\n')

        f.write(f"\n#endif /* {guard_name} */\n")

    log_info(f"Generated master header: {master_header}")
    log_info(f"Found {len(headers)} header files in include/")
