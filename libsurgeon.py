        ...
        if jobs == 1:
            ...
            for i, obj_file in enumerate(obj_files, 1):
                ...
                result = decompile_object_file(
                    obj_file, src_dir, ghidra_headless,
                    decompile_script, project_dir, skip_existing
                )
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result.success:
                            ...
                            print(f"[{i}/{total}] {basename}: {status} ({result.lines} lines)")
                            batch_result.success += 1
                            if result.skipped:
                                batch_result.skipped += 1
                            batch_result.total_lines += result.lines
...
def run_quality_evaluation(src_dir: str, output_dir: str):
    ...
    try:
        # Run evaluation
        result = subprocess.run(
            [sys.executable, eval_script, src_dir, '--json', 
...
"""
    ...
    parser.add_argument(
        '-g', '--ghidra',
        required=True,
        help='Path to Ghidra installation (REQUIRED)'
    )
    parser.add_argument(
        '-o', '--output',
    ...
    parser.add_argument(
        '--list',
        action='store_true',
        help='List file contents without decompiling'
    )
    parser.add_argument(
        '--no-skip',
        action='store_true',
        help='Do not skip already decompiled files'
    )
    parser.add_argument(
        '-c', '--clean',
        action='store_true',
    ...
    if os.path.isfile(args.target):
        ...
        if scan_result.elf_files:
            log_warn(f"Skipping {len(scan_result.elf_files)} ELF files (not supported in Python version)")
